function [delta, kbar, CE_enter, U_enter, trace] = compute_entry_probability_bisect_fp( ...
    cfg, sigma, Fh, fh, supp)
% FP: Endogenous entry δ 
%   h(δ) = Φ((CE(δ)-μ_k)/σ_k) - δ,
% where CE(δ) is computed from the WIN-PROB-WEIGHTED expected utility:
%   U_n = ∫ u(b(c;n)-c) * [1-F(c)]^(n-1) * f(c) dc,  u(x)=-exp(-α x),
...
%   Fh,fh : CDF/PDF handles of FP type c^FP (exact normal recommended)
...
% allow calibrated entry costs if present
...
% cache E[u] per n (independent of δ once b(c;n) fixed)
EU_cache = containers.Map('KeyType','double','ValueType','double');

rootFun = @(d) Phi_of_CE_FP(d, cfg, sigma, N, alpha, muK, sdK, Fh, fh, supp, EU_cache) - d;

% ----- robust bracketing on (eps, 1-eps) -----
...
if abs(fa) < hTol
    delta = epsL;
    [Phi_val, kbar, CE_enter, U_enter] = Phi_of_CE_FP(delta, cfg, sigma, N, alpha, muK, sdK, Fh, fh, supp, EU_cache);
...
if fa*fb < 0
    ...
    % coarse vectorized scan (denser than a few fixed points)
    Kscan = 201;
    gridD = linspace(epsL, epsR, Kscan);
    hvals = arrayfun(@(d) safe_eval(rootFun,d), gridD);

    % treat near-zero as zero for sign
    sgn = sign(hvals);
    sgn(abs(hvals) < hTol) = 0;
    ...
    % sign change between consecutive finite points
    idx = [];
    for i = 1:Kscan-1
    ...
    if ~isempty(idx)
        ...
        % adaptive refinement: around minimal |h|
        ...
        if ~found
            % boundary fallback (δ at boundary)
            if hvals(end) >= 0
                delta = epsR;
                [Phi_val, kbar, CE_enter, U_enter] = Phi_of_CE_FP(delta, cfg, sigma, N, alpha, muK, sdK, Fh, fh, supp, EU_cache);
...
% =================== helpers =================== %
function [Phi_val, kbar, CE, U] = Phi_of_CE_FP(delta, cfg, sigma, N, alpha, muK, sdK, Fh, fh, supp, EU_cache)
% Compute CE(δ) for FP (with win-prob weighting), then Φ((CE-μ)/σ)
...
for n = 2:N
    ...
    if isKey(EU_cache, key)
        ...
        % FP bidding function for this n
        b_handle = build_bfp_handle(cfg, Fh, fh, supp, n);
        % WIN-PROB WEIGHTED utility
        EU_n = expected_utility_integral_weighted(cfg, b_handle, Fh, fh, supp, alpha, n);
        EU_cache(key) = EU_n;
    end
    U = U + pn * EU_n;
end
